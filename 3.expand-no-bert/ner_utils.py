...
def make_data(data_list, tag_dict, word_dict, max_len):
    ...
    for value in tqdm(data_list):
        ...
        for word, tag in zip(token, target):
            token_trans.append(word_dict[word])
    ...
    return torch.LongTensor(token_list), torch.LongTensor(target_list), torch.LongTensor(lens_list)
