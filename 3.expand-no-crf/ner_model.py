...
class MyNER(nn.Module):
    ...
    def __init__(self, batch_size, num_tag, max_len, embed_dim, lstm_dim, lstm_layers, lstm_dropout, dropout):
        ...
        # linear
        self.dropout = nn.Dropout(p=dropout)
        self.linear_lstm = torch.nn.Sequential(torch.nn.Linear(in_features=embed_dim, out_features=lstm_dim),
                                               torch.nn.ReLU())
        # linear-crf
        self.linear_crf = torch.nn.Sequential(torch.nn.Linear(in_features=lstm_dim, out_features=num_tag),
                                              torch.nn.ReLU())
    ...
    def loss_fun(self, feats, tag_tenser, lens):
        # loss
        loss = 0
        lens = lens.cpu().tolist()
        for i in range(self.batch_size):
            loss += self.loss(feats[i][:lens[i]], tag_tenser[i][:lens[i]])
        return loss / self.batch_size

    def predict(self, feats):
        # loss
        output = torch.argmax(feats, dim=2)
        return output
